...
=== Style
In js variable are `snake_case`, function are `camelCase`, types and namespaces
are `BigCamelCase`, additional dynamic properties start with `_` and are
`_camel_case`. In go we follow the standard go convetions.
